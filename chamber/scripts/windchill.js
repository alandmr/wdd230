...
fetch(`${apiEndpoint}?appid=${apiKey}&q=${encodeURIComponent('Barcelona, VE')}&units=imperial`)
  ...
  .then(data => {       
    ...
    icon2 = "https://openweathermap.org/img/wn/"+data.list[5].weather[0].icon+".png"; 

    tempa3 = data.list[13].main.temp;
    tempaF3 = (tempa3 * 9/5)+32;        
    icon3 = "https://openweathermap.org/img/wn/"+data.list[13].weather[0].icon+".png";

    tempa4 = data.list[21].main.temp;
    tempaF4 = (tempa4 * 9/5)+32;        
    icon4 = "https://openweathermap.org/img/wn/"+data.list[21].weather[0].icon+".png";
    ...
    if (tempa <= 50 && wspeed > 3){
        windChill = (35.74+0.6215*tempa)-(35.75*wspeed^0.16)+(0.4275*tempa*wspeed^0.16);
